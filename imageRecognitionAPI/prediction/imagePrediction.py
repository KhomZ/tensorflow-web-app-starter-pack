# ikhomkodes
import numpy as np
from numpy.lib.type_check import imag
from PIL import Image
from io import BytesIO
import tensorflow as tf
# We will create an API to classify images, we name it predict/image.
# We will use Tensorflow for creating the image classification model.

from tensorflow.keras.applications import imagenet_utils
from tensorflow.keras.applications.imagenet_utils import decode_predictions


# We create a function load_model,
# which will return a MobileNet CNN Model with pre-trained weights
# i.e. it is already trained to classify 1000 unique categories of images.
def load_model():
    model = tf.keras.applications.MobileNetV2(weights="imagenet")
    print("Model loaded")
    return model


# model = load_model()
model = None


# Now, we define a predict function that will accept an image and returns the predictions.
# We resize the image to 224x224 and normalize the pixel values to be in [-1, 1].
# decode_predictions is used to decode the class name of the predicted object.
# Here we will return the top-2 probable class.
def predict(image: Image.Image):
    global model
    if model is None:
        model = load_model()

    image = np.asarray(image.resize((224, 224)))[..., :3]
    image = np.expand_dims(image, 0)
    image = image / 127.5 - 1.0

    result = decode_predictions(model.predict(image), 2)[0]

    response = []
    for i, res in enumerate(result):
        resp = {}
        resp["class"] = res[1]
        resp["confidence"] = f"{res[2]*100:0.2f} %"

        response.append(resp)

    return response


# Now we will create an API /predict/image that supports file upload.
# We will filter the file extension to support only jpg, jpeg, and png format of images.
# We will use Pillow to load the uploaded image.
def read_imagefile(file) -> Image.Image:
    image = Image.open(BytesIO(file))
    return image
